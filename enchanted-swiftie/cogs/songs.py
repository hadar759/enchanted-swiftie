...
class Songs(commands.Cog):
    ...
    async def command_singles(self, ctx: commands.Context):
        ...
        while self.last_checked_billboard + dt.timedelta(weeks=1) <= dt.datetime.date(dt.datetime.now() + dt.timedelta(weeks=1)):
    ...
    def add_singles_to_dict(self, chart: billboard.ChartData):
        ...
        for entry_place, entry in enumerate(chart):
            if entry.artist == "Taylor Swift":
                ...
                if entry.title not in self.singles_dict or entry_place < int(self.singles_dict[entry.title]):
    ...
    def read_singles_info(self):
        ...
        with open(r"resources\billboard.txt", "r") as singles_file:
            ...
            self.singles_dict = dict(single.rstrip("\n").split(" - ") for single in singles_file.readlines())
